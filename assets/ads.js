...
(function (global) {
  ...
  const HOUSE = [
    { id:"awraqna", live:true, url:"https://awraqna.com", badge:"أوراقنا",
      title_ar:"أوراقنا — أوراق عمل رياضيات للطباعة", title_en:"Awraqna — Printable Math Worksheets",
      body_ar:"ولّد ورقة عمل جديدة بأسئلة عشوائية وصفحة إجابات — مجاناً.",
      body_en:"Generate a fresh worksheet with an answer key — free.",
      cta_ar:"ولّد ورقتك", cta_en:"Generate now",
      c1:"#241a10", c2:"#5a3d1c", accent:"#f0b264" },
    { id:"art", live:true, url:"https://altayebamer.com", badge:"الطيب عامر",
    ...
    { id:"mawlidi", live:true, url:"https://mawlidi.com/arb/", badge:"مَوْلِدي",
    ...
    { id:"kashf", live:false, url:"#", badge:"كشف",
    ...
    { id:"quran", live:true, url:"https://qurankarem.org", badge:"القرآن الكريم",
    ...
    { id:"seven", live:false, url:"#", badge:"The Seven Council",
  ...
  /* لا نعرض إلا الإعلانات التي لها رابط حقيقي يعمل.
     إعلان يقود إلى "#" أو إلى نطاق غير مسجَّل يضرّ بالمصداقية أكثر
     مما ينفع — فالأفضل ألا تظهر مساحة إعلانية أصلاً. */
  const LIVE = HOUSE.filter(a => a.live && a.url && a.url !== "#");
  let seed = Math.floor(Math.random() * Math.max(1, LIVE.length));
  function pick() { const a = LIVE[seed % LIVE.length]; seed++; return a; }
  ...
  function render(slot, size) {
    if (!LIVE.length) { slot.style.display = "none"; return; }   // لا إعلانات حيّة → أخفِ المساحة
    const a = pick();
    const en = T();
